...
Juegos Din\'amicos con Informaci\'on Incompleta}   
...
\item<3-> Anteriormente hemos supuestos distintos casos, pero vamos a explorar si \textit{en equilibrio} podemos encontrar una soluci\'on consistente.
\end{wideitemize}
\end{frame}
...
edge from parent node[left,xshift=-10]{$\text{w}$}
}
...
\begin{frame}{Introducci\'on}{Recordando sets de informaci\'on (y sets de sets de informaci\'on)}
	\begin{wideitemize}
		\item<1-> $H_{i}$ es el set de sets de informaci\'on del jugador $i$. Por lo tanto, $h_{i} \in H_{i}$ es un set de informaci\'on.
		\item<2-> Dentro de un set de informaci\'on es posible tener una o m\'as historias no terminales. Por lo tanto, para $h,h' \in h_{i}$, se tiene que $A(h)=A(h')$. 
		\item<3-> Para $h_{i} \in H_{i}$, denotamos con $A(h_{i})$ el set $A(h)$ y $P(h_{i})$ el set $P(h)$. 
	\end{wideitemize}
\end{frame}
...
\begin{frame}{Introducci\'on}{Ejemplo 1}
	\begin{wideitemize}
		...
		\begin{align*}
			H_{E} &= \{ \{\emptyset\} \} \\
			H_{I} &= \{ \{ e1, e2 \} \}
		\end{align*} donde $H_{E}$ y $H_{I}$ son los sets de sets de informaci\'on de ambos jugadores. Por lo tanto, una estrategia $S_{i}$ es una funci\'on 
		\begin{align*}
		S_{i}: H_{i} \rightarrow \times_{\forall h_{i}\in H_{i}} A(h_{i})
		\end{align*} donde $h_{i}$ es un set de informaci\'on.
	\end{wideitemize}
\end{frame}

\begin{frame}{Perfect Bayesian Equilibrium}
	\begin{wideitemize}
		\item<1-> Belief System\footnote{Sistema de creencias} es una funci\'on $\mu(h_{i})(h)$ que indica la probabilidad que el jugador $P(h_{i})$ asigna a la historia $h \in h_{i}$, condicional a que el juego llegue a $h_{i}$.
		\item<2-> Definimos un assesstment como $(s,\mu)$, donde $s=(s_{1},\dots s_{N})$ y $\mu=(\mu_{1},\dots, \mu_{N})$.
		\item<3-> Un perfil de assesstments $(s,\mu)$ es un Perfect Bayesian Equilibrium si
		\begin{wideitemize}
			\item<4->[--] Para cada $i$ y todo $h_{i} \in H_{i}$, $s_{i}$ es secuencialmente racional (maximiza el beneficio esperado una vez en $H_{i}$ y dado $\mu_{i}$ y $s_{-i}$).
...
\begin{wideitemize}
	...
	\item<2-> Sabemos que dado el set de informaci\'on $h_{I} = \{ e1, e2 \}$, este jugador para poder decidir $P$ o $NP$ debe suponer probabilidades sobre que historia ha ocurrido. 
	...
	\item<4-> La pregunta es entonces, qu\'e probabilidad asigna entonces a $\mu(h_{I})(e1)$?\footnote{Como $h_{I}$ solo tiene dos elementos, $\mu(h_{I})(e2) = 1 - \mu(h_{I})(e1)$.}
\end{wideitemize}
\end{frame}

\begin{frame}{Perfect Bayesian Equilibrium}{Ejemplo 1a}
\begin{wideitemize}
	\item<1-> ¿Existe un equilibrio donde $s_{E} = e2$?
	\begin{wideitemize}
		\item<2->[--] Primero. Ver FIGURA.
		\item<3->[--] Segundo. Aplicar regla de Bayes si es posible. Asume que e1 se da con prob p, e2 con prob q y s con prob 1-p-q. 
		%Adicionalmente, hay solo un tipo para $E$.
		\begin{align*}
			\mu = & \frac{ p }{ p + q } 
		\end{align*} pero si $s_{E} = e2$, entonces $\mu = 0$.
		\item<4->[--] Tercero. Respuestas \'optimas. Entonces $s_{I} = NP$.
		\item<5->[--] Cuarto. Acciones \'optimas. E no tiene incentivos a desviarse de $s_{E} = e2$.
	\end{wideitemize}
	\item<6-> Entonces $s_{E} = e2$, $s_{I} = NP$ y $\mu = 0$ es un equilibrio perfecto bayesiano.
\end{wideitemize}	
\end{frame}
...
\begin{frame}{Signalling Games}{Ejemplo 2a - Una version de Spence (1973) modelo de mercado laboral}
	\begin{wideitemize}
		...
		\item<3-> Segundo. Aplicar regla de Bayes si es posible. Notar que 2 tiene una particion $H_{2}$ compuesta por 2 elementos $h_{2}^{1} = \{(PA,E), (PB,E)\}$ y $h_{2}^{2} = \{(PA,NE),(PB,NE)\}$. Asumamos que $\mu(h_{2}^{1})((PA,E))$ es la probabilidad de que estando en $h_{2}^{1}$ se de $(PA,E)$ y $\rho(h_{2}^{2})((PA,NE))$ es la probabilidad de que estando en $h_{2}^{2}$ se de $(PA,NE)$. Adicionalmente, especificamos $\alpha^{PA}$ y $\alpha^{PB}$ como las probabilidades de que el jugador 1 decida estudiar asumiendo la naturaleza eligio PA o PB. 
	\end{wideitemize}
\end{frame}
...
\begin{frame}{Signalling Games}{Ejemplo 2a - Una version de Spence (1973) modelo de mercado laboral}
	\begin{wideitemize}
		...
		\begin{wideitemize}
			\item<2->[--] Bajo $h_{2}^{1}$ y $\mu = 1$. Prefiere M sobre A.
			\item<3->[--] Bajo $h_{2}^{2}$ y $\rho = 0$. Prefiere A sobre M.
		\end{wideitemize}
...
\begin{frame}{Signalling Games}{Ejemplo 2b - Una version de Spence (1973) modelo de mercado laboral}
	\begin{wideitemize}
		\item<1-> Tercero. Respuestas \'optimas. ¿C\'omo responde el jugador 2 dada las acciones de 1 y sus valores de $\mu$ y $\rho$? 
		\begin{wideitemize}
			\item<2->[--] Bajo $h_{2}^{1}$ y $\mu \in [0,1]$. 
			\begin{align*}
				UE_{2}(M;h_{2}^{1},\mu) = & 10\mu + 0(1-\mu) = 10\mu \\
				UE_{2}(A;h_{2}^{1},\mu) = & 4\mu + 4(1-\mu) = 4
			\end{align*} y elegir M es preferible si $UE_{2}(M;\mu) \geq UE_{2}(A;\mu)$ o $\mu \geq 2/5$.
			\item<3->[--] Bajo $I_{2}^{2}$ y $\rho = 1/3$.
			\begin{align*}
				UE_{2}(M;h_{2}^{2},\rho) = & 10(1/3) + 0(2/3) = 10/3 \\
				UE_{2}(A;h_{2}^{2},\rho) = & 4(1/3) + 4(2/3) = 4
...
\begin{frame}{Signalling Games}{Ejemplo 2b - Una version de Spence (1973) modelo de mercado laboral}
	\begin{wideitemize}
		...
		\end{wideitemize} por lo tanto $s_{1} = (NE,NE)$, $s_{2} = (A,A)$, $\mu < 2/5$ y $\rho=1/3$ es un equilibrio perfecto bayesiano.
	\end{wideitemize}
\end{frame}
