function [obj] = E07_gmm_MCFC(param,data_moments,g_mu,g_sigma,...
    mc_grid,profit_mat,weight_mat,FC_draws,subsidy_vec,numsimFC,...
    simTrader)
...
varmat = [sigma1^2,rho*sigma1*sigma2;rho*sigma1*sigma2,sigma2^2];
d=eig(varmat);
...
entry_modmom1 = mean(mean(entry_sub1.*importance_grid,2),1);
entry_modmom2 = mean(mean(entry_sub2.*importance_grid,2),1);
entry_modmom3 = mean(mean(entry_sub3.*importance_grid,2),1);

mc_modmom1 = mean(mean(conditional_mc_sub1.*importance_grid,2),1);
mc_modmom2 = mean(mean(conditional_mc_sub2.*importance_grid,2),1);
mc_modmom3 = mean(mean(conditional_mc_sub3.*importance_grid,2),1);


modmom = [entry_modmom1;entry_modmom2;entry_modmom3;mc_modmom1;mc_modmom2;mc_modmom3];

moments = data_moments'-modmom;


obj = moments'*weight_mat*moments;
